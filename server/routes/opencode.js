/**
 * opencode.js — OpenCode Agent endpoint (optimizado para codigo)
 *
 * Endpoints:
 *   POST /api/opencode         -> chat con SSE streaming (formato codigo)
 *   GET  /api/opencode/status  -> estado del modelo
 *
...
function getOpenCodeModel() {
  return process.env.OPCODE_MODEL || 'negentropy-claude-opus-4.7-9b';
}
...
async function chatDispatch({ messages, model, stream, maxTokens, temperature, topP }) {
  const m = model || getOpenCodeModel();
...
router.post('/', async (req, res) => {
  ...
  const model   = reqModel || getOpenCodeModel();
  const provider = getProvider(model);
  const system  = customSystem || `Eres OpenCode, un agente de código especializado del sistema SynK-IA.\nTu tarea es generar, revisar, refactorizar y explicar código.\nResponde en español, pero genera el código en el idioma/lenguaje que corresponda.\nSe preciso, eficiente y usa buenas prácticas de programación.\nCuando generes código, asegúrate de que sea funcional y esté bien documentado.`;
