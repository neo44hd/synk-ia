...
//
// AIDEN se conecta directamente a ws://localhost:18789 sin handshake ni auth
// y funciona perfectamente (loopback). Este proxy hace lo mismo: conexión
// transparente sin interceptar el protocolo de connect.challenge.
// ─────────────────────────────────────────────────────────────────────────────
...
export function setupOpenClawProxy(httpServer) {
  ...
  wss.on('connection', (clientWs) => {
    ...
    let upstreamWs;
    try {
      // Conexión simple sin headers de auth — igual que AIDEN
    ...
    upstreamWs.on('open', () => {
      ...
      clientWs.send(JSON.stringify({ type: 'connected', sessionId: 'proxy' }));
    ...
    // Upstream (OpenClaw) → Cliente (browser) — reenvío transparente
    ...
    // Cliente (browser) → Upstream (OpenClaw) — reenvío transparente
